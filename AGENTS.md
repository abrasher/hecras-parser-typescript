...
This repo is migrating to a schema‑first parser/serializer. The guidance below distills what we’ve learned while implementing schemas so far and should prime agents to make consistent, correct changes quickly.

Read these first:

- `docs/tasks/schema-first-migration.md` — plan, milestones, parity goals, decisions/risks
...
- Keep type source‑of‑truth as the schema: use `Infer<typeof mySchema>` for model typing; optionally `satisfies` against existing models during migration.
...
- Treat `src/parsers/**` and `src/serializers/**` as read‑only except for critical blockers. New logic belongs in `src/schemas/**` + DSL.
- Keep top‑level parsing tolerant (non‑strict) until full migration; drive composition with recognizers (`startsWith(...)`) and adapters as tracked in the migration doc.
...
- `src/parsers/utils.ts` has shared helpers (`parseKeyValue`, `parseMultilineArray`, `splitIntoTuples`). We can reuse these from `contextual` items, but do not expand legacy parsers.
...
- Do not add new logic to `src/parsers/**` or `src/serializers/**` except critical fixes to unblock migration.
- Keep schemas tolerant (non‑strict) at the edges while we still mix legacy and new sections.
- If ordering or formatting in serializers forces compromises, document under “Decisions & Risks” in `docs/tasks/schema-first-migration.md` and mention in PR notes.

## Checklist: Adding A New Schema

1) Read existing model + any legacy parser/serializer to catalog fields and formats.
...
6) Update `docs/tasks/schema-first-migration.md` with coverage and any decisions/risks.
...
- Capture the issue under “Decisions & Risks” in `docs/tasks/schema-first-migration.md` with a minimal reproducible snippet.
- Prefer `contextual` hooks to bridge format edge cases without expanding legacy code.
