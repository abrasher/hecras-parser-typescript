...
/**
 * Script to compare Muncie.g01 file with its round-trip serialized version
...
function main() {
  console.log("=== Muncie.g01 Round-trip Line-by-Line Comparison ===\n")
  ...
  try {
    ...
    const serializedOutputPath = "test/data/Muncie.serialized.g01"
    ...
    if (serializedLines.length < originalLines.length) {
      ...
      for (
        let i = serializedLines.length;
        i < Math.min(serializedLines.length + 10, originalLines.length);
        i++
      ) {
