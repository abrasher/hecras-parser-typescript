...
END GEOM DESCRIPTION:`
  ...
  it("should parse multi-line description", () => {
    ...
    expect(result.data.description).toBe("Line 1 of description\nLine 2 of description\nLine 3 of description")
  })
  ...
  it("should handle empty description block", () => {
    const testLines = ["Geom Title=Empty Description", "BEGIN GEOM DESCRIPTION:", "END GEOM DESCRIPTION:"]
    const result = parseHeader(testLines, 0)
  ...
  it("should stop parsing at first non-header line", () => {
    const testLines = ["Geom Title=Test", "Program Version=6.60", "Storage Area=First Non-Header", "More Content"]
    const result = parseHeader(testLines, 0)
