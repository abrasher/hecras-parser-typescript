...
} from "../../src/serializers/geometry/bridgeSerializer"
...
describe("Bridge Serializer Tests", () => {
  // Test data from ConnectionBridge.test.ts
...
Conn BR: DSXS Ineff=11.68,252.5,30,252.5`
  ...
  describe("serializeBridgeConfiguration", () => {
    it("should serialize bridge configuration", () => {
      const result = serializeBridgeConfiguration(testBridgeData.bridge)

      expect(result).toEqual(["Conn BR: Bridge=-1,0,-1,-1, 0 ,0.3,0.5"])
    })
  })
  ...
  describe("serializeBridgeCoefficients", () => {
    it("should serialize bridge coefficients with nulls", () => {
      const result = serializeBridgeCoefficients(testBridgeData.bridgeCoefficients)

      expect(result).toEqual(["Conn BR: BR Coef=-1 , 0 , 0 ,,,0.8,0,,0,"])
    })
  })

  describe("serializeIneffectiveFlowArea", () => {
    it("should serialize upstream ineffective flow area", () => {
      const result = serializeIneffectiveFlowArea(testBridgeData.upstreamIneffectiveFlowArea, "USXS")

      expect(result).toEqual(["Conn BR: USXS Ineff=10.68,252.93,29,253.8"])
    })

    it("should serialize downstream ineffective flow area", () => {
      const result = serializeIneffectiveFlowArea(testBridgeData.downstreamIneffectiveFlowArea, "DSXS")

      expect(result).toEqual(["Conn BR: DSXS Ineff=11.68,252.5,30,252.5"])
    })
  })

  describe("serializeBridgeCrossSection", () => {
    it("should serialize bridge cross section", () => {
      const section = testBridgeData.insideUpstreamCrossSection
      const result = serializeBridgeCrossSection(section, "BR")

      expect(result[0]).toBe("Conn BR: BR SE=1,62")
      expect(result[1]).toBe("       0 252.093    .584 252.088    .789 252.069   1.108 252.019   2.156 251.769")
      expect(result[result.length - 3]).toBe("Conn BR: BR Bank Stations=1,7.575,30.44")
      expect(result[result.length - 2]).toBe("Conn BR: BR Mann=1,3")
      expect(result[result.length - 1]).toBe("       0     .09   7.575    .035  33.923     .09")
    })

    it("should serialize external cross section", () => {
      const section = testBridgeData.externalUpstreamCrossSection
      const result = serializeBridgeCrossSection(section, "XS")

      expect(result[0]).toBe("Conn BR: XS SE=1,58")
      expect(result[result.length - 3]).toBe("Conn BR: XS Bank Stations=1,7.237,30.193")
      expect(result[result.length - 2]).toBe("Conn BR: XS Mann=1,3")
    })
  })

  describe("serializeBridgeConnection", () => {
    it("should serialize complete bridge connection", () => {
      const result = serializeBridgeConnection(testBridgeData)
  ...
  describe("round-trip tests", () => {
    it("should parse and serialize back to identical format", () => {
      ...
      const parsed = parseBridgeData(lines[0], lines, 0)
      const serialized = serializeBridgeConnection(parsed.data)
    ...
    it("should parse and serialize back to identical data", () => {
      ...
      const parsed = parseBridgeData(lines[0], lines, 0)
      const serialized = serializeBridgeConnection(parsed.data)
      const reParsed = parseBridgeData(serialized[0], serialized, 0)
