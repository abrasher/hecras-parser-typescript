...
describe("StorageAreaSchema", () => {
  describe("2D Storage Area parsing and serialization", () => {
    const testStorageArea2D: StorageAreaSchema = {
      id: "2D_Grid",
      centroidX: null,
      centroidY: null,
      surfaceLine: [
    ...
    it("should parse storage area from lines", () => {
      const result = parseSectionWithSchema(storageAreaSchema, inputLines, 0)

      expect(result.value.id).toBe("2D_Grid")
      expect(result.value.centroidX).toBeNull()
      ...
      expect(result.value.points2D).toEqual(testStorageArea2D.points2D)
      expect(result.value.pointsPerimeterTime).toBe("21May2025 13:18:09")
      expect(result.value.mannings).toBe(0.06)
      expect(result.value.cellVolumeFilterTolerance).toBe(0.01)
      expect(result.value.locked).toBe(-1)
    })

    it("should serialize storage area to lines", () => {
      ...
      expect(result).toContain("Storage Area Point Generation Data=,,25,25")
      expect(result).toContain("Storage Area 2D Points= 8 ")
      expect(result).toContain("Storage Area 2D PointsPerimeterTime=21May2025 13:18:09")
      expect(result).toContain("Storage Area Mannings=0.06")
      expect(result).toContain("2D Locked=-1")
    })

    it("should round-trip parse and serialize", () => {
      const parsed = parseSectionWithSchema(storageAreaSchema, inputLines, 0)
      const serialized = serializeWithSchema(storageAreaSchema, parsed.value)
      const reparsed = parseSectionWithSchema(storageAreaSchema, serialized, 0)
