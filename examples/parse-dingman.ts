...
async function main() {
  console.log('🚀 Parsing Dingman.g01 with HECRASParserV2')
  ...
  try {
    ...
    console.log(`   Viewing Rectangle: ${JSON.stringify(geometry["Viewing Rectangle"])}`)
    ...
    if (geometry.storageAreas.length === 0) {
      ...
      geometry.storageAreas.forEach((storageArea, index) => {
        ...
        console.log(`   ├─ Position: ${storageArea.centroid ? `(${storageArea.centroid.x}, ${storageArea.centroid.y})` : 'N/A'}`)
        console.log(`   ├─ Manning's n: ${storageArea.mannings}`)
        console.log(`   ├─ Area: ${storageArea.area || 'N/A'} sq ft`)
        console.log(`   ├─ Min Elevation: ${storageArea.minElevation || 'N/A'} ft`)
        ...
        if (storageArea.surfaceLine && storageArea.surfaceLine.length > 0) {
          console.log(`   ├─ Surface Line Points: ${storageArea.surfaceLine.length}`)
          console.log(`   │  First: (${storageArea.surfaceLine[0].x}, ${storageArea.surfaceLine[0].y})`)
          if (storageArea.surfaceLine.length > 1) {
            const last = storageArea.surfaceLine[storageArea.surfaceLine.length - 1]
        ...
        if (storageArea.volumeElevationData && storageArea.volumeElevationData.length > 0) {
          console.log(`   ├─ Volume-Elevation Points: ${storageArea.volumeElevationData.length}`)
          const firstVE = storageArea.volumeElevationData[0]
          console.log(`   │  First: Elev ${firstVE.elevation} ft → Vol ${firstVE.volume} cu ft`)
          if (storageArea.volumeElevationData.length > 1) {
            const lastVE = storageArea.volumeElevationData[storageArea.volumeElevationData.length - 1]
            console.log(`   │  Last: Elev ${lastVE.elevation} ft → Vol ${lastVE.volume} cu ft`)
          }
        } else {
        ...
        if (typeof storageArea.toGeoJSON === 'function') {
          try {
            ...
            console.log(`   └─ GeoJSON: ${geoJson.type} with ${geoJson.geometry?.coordinates?.length || 0} coordinate groups`)
          } catch (error) {
            console.log(`   └─ GeoJSON: Error generating (${error instanceof Error ? error.message : String(error)})`)
          }
        } else {
    ...
    if (geometry.reaches.length > 0) {
      ...
      geometry.reaches.forEach((reach, index) => {
        console.log(`   Reach #${index + 1}: ${reach.riverName} - ${reach.reachName}`)
        console.log(`   ├─ Centerline Points: ${reach.centerline?.length || 0}`)
        console.log(`   ├─ Cross Sections: ${reach.crossSections?.length || 0}`)
        console.log(`   └─ Lateral Structures: ${reach.lateralStructures?.length || 0}`)
      })
    }
