...
async function debugParser() {
  console.log("🔍 Debug: HECRASParser parsing")
  ...
  try {
    ...
    const { GeometryParserPlugin } = await import(
      "../src/plugins/geometryPlugin.js"
    )
    ...
    if (tokenResult.data && tokenResult.data.length > 0) {
      ...
      tokenResult.data.slice(0, 10).forEach((token, i) => {
        console.log(
          `  ${i + 1}. ${token.type}: "${token.content}" (line ${token.lineStart})`,
        )
        if (token.metadata) {
