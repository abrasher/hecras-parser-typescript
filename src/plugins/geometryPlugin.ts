import type { FileTypeDetector, SectionHandler } from "../core/plugins";
import { BaseParserPlugin } from "../core/plugins";
import type { TokenizerConfig } from "../core/tokenizer";
import type { ParsedToken, ParseContext, ParseResult} from "../core/pipeline";
import { TokenType } from "../core/pipeline";
import type { ValidationSchema} from "../core/validator";
import { CommonValidationRules } from "../core/validator";
import type { ModelMapping} from "../core/modelBuilder";
import { BaseModelBuilder } from "../core/modelBuilder";
...
export class GeometryParserPlugin extends BaseParserPlugin {
    ...
    detector: FileTypeDetector = this.createDetector([
        ...
        /River Reach=/i,
        /Connection=/i,
    ], ["g01", "g02", "g03", "g04", "g05"], 0.9, "geometry");
    ...
    private createUniversalHandler(): SectionHandler {
        return this.createSectionHandler(["universal"], 100, async (tokens: ParsedToken[], context: ParseContext): Promise<ParseResult<any>> => {
            ...
            for (let i = 0; i < tokens.length; i++) {
                ...
                else if (token.type === TokenType.SECTION_HEADER &&
                ...
                if (key && value) {
                    ...
                    // Cross Section processing
                        ...
                        if (token.metadata?.hasData && token.content.includes("\n")) {
                            ...
                            for (let j = i + 1; j < tokens.length && !foundData; j++) {
                                ...
                                if (nextToken.type === TokenType.SECTION_HEADER ||
                    ...
                    else if (key === "Storage Area Surface Line" &&
                        currentStorageArea) {
                        if (token.metadata?.hasData && token.content.includes("\n")) {
                            const lines = token.content.split("\n").slice(1);
                            const coordResult = HECRASPrimitives.parseMultilineCoordinates(lines);
    ...
    private createHeaderHandler(): SectionHandler {
        return this.createSectionHandler(["header"], 100, async (tokens: ParsedToken[], context: ParseContext): Promise<ParseResult<any>> => {
            const result: any = {};
            const errors: any[] = [];
            const warnings: any[] = [];
            // Only process header-specific tokens, leave others for other handlers
            const headerTokens = tokens.filter((token) => token.type === TokenType.KEY_VALUE &&
                token.metadata?.key &&
        ...
        (tokens: ParsedToken[]) => {
            const hasHeaderTokens = tokens.some((token) => token.type === TokenType.KEY_VALUE &&
                token.metadata?.key &&
                ["Geom Title", "Program Version", "Viewing Rectangle"].includes(token.metadata.key));
            if (hasHeaderTokens) {
    ...
    private createReachHandler(): SectionHandler {
        return this.createSectionHandler(["River Reach"], 90, async (tokens: ParsedToken[], context: ParseContext): Promise<ParseResult<any>> => {
            const reaches: any[] = [];
            const errors: any[] = [];
            const warnings: any[] = [];
    ...
    private createCrossSectionHandler(): SectionHandler {
        return this.createSectionHandler(["Type RM Length L Ch R"], 85, async (tokens: ParsedToken[], context: ParseContext): Promise<ParseResult<any>> => {
    ...
    private createStorageAreaHandler(): SectionHandler {
        return this.createSectionHandler(["Storage Area"], 80, async (tokens: ParsedToken[], context: ParseContext): Promise<ParseResult<any>> => {
            const storageAreas: any[] = [];
            const errors: any[] = [];
            const warnings: any[] = [];
            let currentStorageArea: any = null;
            // First, collect all storage area related tokens
            const storageAreaTokens = tokens.filter((token) => token.type === TokenType.KEY_VALUE &&
        ...
        (tokens: ParsedToken[]) => {
            ...
            const keys = kvTokens
                ...
                .filter((k) => k);
            const hasStorageAreaTokens = kvTokens.some((token) => token.metadata?.key?.startsWith("Storage Area"));
    ...
    private createConnectionHandler(): SectionHandler {
        return this.createSectionHandler(["Connection"], 75, async (tokens: ParsedToken[], context: ParseContext): Promise<ParseResult<any>> => {
            ...
            for (const token of tokens) {
                if (token.type === TokenType.KEY_VALUE) {
                ...
                else if (token.type === TokenType.COORDINATE_DATA &&
                    currentConnection) {
                    ...
                    currentConnection.weirStationElevation.push(...stationElevResult.data);
                    warnings.push(...stationElevResult.warnings);
                }
            }
            return {
    ...
    private parseConnectionProperty(connection: any, key: string, value: string): void {
        switch (key) {
    ...
    private createBoundaryConditionHandler(): SectionHandler {
        return this.createSectionHandler(["BC Line Name"], 70, async (tokens: ParsedToken[], context: ParseContext): Promise<ParseResult<any>> => {
    ...
    private createGISHandler(): SectionHandler {
        return this.createSectionHandler(["LCMann Time", "Geom Raster"], 60, async (tokens: ParsedToken[], context: ParseContext): Promise<ParseResult<any>> => {
    ...
    private createReachValidationSchema(): ValidationSchema<any> {
        return {
            rules: [
                ...
                CommonValidationRules.validCoordinates("coordinates", (data) => data.coordinates),
            ],
        };
    }
    private createCrossSectionValidationSchema(): ValidationSchema<any> {
        return {
            rules: [
                CommonValidationRules.positiveNumber("riverMile", (data) => data.riverMile),
                CommonValidationRules.validCoordinates("coordinates", (data) => data.coordinates),
            ],
        };
    }
    private createStorageAreaValidationSchema(): ValidationSchema<any> {
        return {
            rules: [
                ...
                CommonValidationRules.validCoordinates("surfaceLine", (data) => data.surfaceLine),
                CommonValidationRules.positiveNumber("mannings", (data) => data.mannings),
            ],
        };
    }
    private createConnectionValidationSchema(): ValidationSchema<any> {
        return {
            rules: [
                ...
                CommonValidationRules.positiveNumber("weirWidth", (data) => data.weirWidth),
            ],
        };
    }
}
