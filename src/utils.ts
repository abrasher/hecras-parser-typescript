...
export function formatCoordinatesToString(
  ...
  for (let i = 0; i < coords.length; i++) {
    ...
    const xStr = formatNumber(coord.x, coordPrecision).padStart(
      coordPrecision === 2 ? 17 : 18,
      " ",
    ) // Adjust padding
    const yStr = formatNumber(coord.y, coordPrecision).padStart(
      coordPrecision === 2 ? 17 : 18,
      " ",
    )
...
export function formatManningSegmentsToString(
  segments: ManningSegment[],
  precision: number = 2,
): string[] {
  const lines: string[] = []
  let currentLine = ""
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const nValStr = formatNumber(
      seg.nValue,
      precision === 2 ? 3 : precision + 1,
    ).padStart(precision === 2 ? 8 : 9, " ") // .07 needs 3 for nVal
