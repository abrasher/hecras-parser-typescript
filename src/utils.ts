...
export function parseKeyValue(line: string, separator: string = '='): { key: string, value: string } | null {
    ...
    if (parts.length >= 2) {
        return { key: parts[0].trim(), value: parts.slice(1).join(separator).trim() };
    }
    return null;
}
...
export function formatCoordinatesToString(coords: Coordinate[], itemsPerLine: number = 2, coordPrecision: number = 2): string[] {
    const lines: string[] = [];
    let currentLine = "";
...
export function formatStaElevToString(points: StationElevationPoint[], itemsPerLine: number = 5, precision: number = 2): string[] {
    const lines: string[] = [];
    let currentLine = "";
...
export function formatVolumeElevationToString(points: VolumeElevationPoint[], itemsPerLine: number = 5, precision: number = 2): string[] {
    const lines: string[] = [];
    let currentLine = "";
    for (let i = 0; i < points.length; i++) {
        const point = points[i];
...
export function formatManningSegmentsToString(segments: ManningSegment[], precision: number = 2): string[] {
    ...
    for (let i = 0; i < segments.length; i++) {
     ...
     if (currentLine.trim() !== "") { // Catch any remaining part
        lines.push(currentLine.trimEnd());
    }
    return lines;
}