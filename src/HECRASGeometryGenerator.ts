...
import {
    Coordinate, StationElevationPoint, ManningSegment, VolumeElevationPoint
} from './models/common';
import {
    HECRASGeometry, ViewingRectangle, GisInfo
...
import {
    formatNumber, formatCoordinatesToString, formatStaElevToString,
    formatManningSegmentsToString, formatVolumeElevationToString
...
export class HecRasFileGenerator {
    ...
    private generateHeader(): void {
        ...
        if (this.geometry.viewingRectangle) {
            ...
            this.addLine(`Viewing Rectangle= ${formatNumber(vr.minX, DEFAULT_COORD_PRECISION)} , ${formatNumber(vr.maxY, DEFAULT_COORD_PRECISION)} , ${formatNumber(vr.maxX, DEFAULT_COORD_PRECISION)} , ${formatNumber(vr.minY, DEFAULT_COORD_PRECISION)} `);
        }
    ...
    private generateReach(reach: Reach): void {
        ...
        if (reach.centerline.length > 0) {
            ...
            this.addLines(formatCoordinatesToString(reach.centerline, 2, DEFAULT_COORD_PRECISION));
        }
        if (reach.textPosition) {
            this.addLine(`Rch Text X Y=${formatNumber(reach.textPosition.x, DEFAULT_COORD_PRECISION)},${formatNumber(reach.textPosition.y, DEFAULT_COORD_PRECISION)}`);
        }
    ...
    private generateCrossSection(xs: CrossSection): void {
        ...
        this.addLine(`Type RM Length L Ch R = 1 ,${formatNumber(xs.riverStation, DEFAULT_LENGTH_PRECISION)},${l},${ch},${r}`);

        if (xs.gisCutLine.length > 0) {
            this.addLine(`XS GIS Cut Line=${xs.gisCutLine.length}`);
            this.addLines(formatCoordinatesToString(xs.gisCutLine, 2, DEFAULT_COORD_PRECISION)); // HEC-RAS can have 2 or more per line
        }
        ...
        if (xs.staElevData.length > 0) {
            ...
            this.addLines(formatStaElevToString(xs.staElevData, 5, DEFAULT_ELEV_PRECISION));
        }
        if (xs.manningSegments.length > 0) {
            ...
            this.addLines(formatManningSegmentsToString(xs.manningSegments, DEFAULT_MANNING_PRECISION));
        }
        if (xs.bankStations.left !== null && xs.bankStations.right !== null) {
            this.addLine(`Bank Sta=${formatNumber(xs.bankStations.left, DEFAULT_ELEV_PRECISION)},${formatNumber(xs.bankStations.right, DEFAULT_ELEV_PRECISION)}`);
        }
        // Assuming default for XS Rating Curve and HTab for now, add if parsed
        this.addLine("XS Rating Curve= 0 ,0");
        if (xs.staElevData.length > 0) { // Try to get a reasonable starting elevation for HTab
            const minElev = Math.min(...xs.staElevData.map(p => p.elevation));
            this.addLine(`XS HTab Starting El and Incr=${formatNumber(minElev + 0.01, 2)},0.5, 100 `); // Approximate
        } else {
            this.addLine(`XS HTab Starting El and Incr=0,0.5, 100 `); // Fallback
        }
        this.addLine("XS HTab Horizontal Distribution= 5 , 5 , 5 ");
        this.addLine(`Exp/Cntr=${formatNumber(xs.expansionCoefficient, 1)},${formatNumber(xs.contractionCoefficient, 1)}`);
        ...
        if (xs.ineffectiveFlowAreas.length > 0) {
            ...
            xs.ineffectiveFlowAreas.forEach(ineff => {
                ...
                const staStr = formatNumber(ineff.station, DEFAULT_ELEV_PRECISION).padStart(8, ' ');
                const elevStr = formatNumber(ineff.elevation, DEFAULT_ELEV_PRECISION).padStart(8, ' ');
                ...
                this.addLine(`${staStr}${elevStr}${formatNumber(ineff.elevation, DEFAULT_ELEV_PRECISION)}${ineff.isPermanent ? "18" : ""}`); // Approximation
    ...
    private generateLateralStructure(ls: LateralStructure): void {
        this.addLine(`Type RM Length L Ch R = 6 ,${formatNumber(ls.riverStation,0)}   ,,,`); // Lengths usually blank for LS
        ...
        this.addLine(`Lateral Weir End=                ,                ,        ,${ls.associatedRiverStation || "150"}             `);
        ...
        if (ls.stationElevationData.length > 0) {
            ...
            this.addLines(formatStaElevToString(ls.stationElevationData, 2, DEFAULT_ELEV_PRECISION)); // Often 2 pairs per line
        }
        this.addLine(`Lateral Weir Centerline= 0 `);
        this.addLine(`Lateral Weir HW RS Station=${ls.associatedRiverStation || formatNumber(ls.riverStation, DEFAULT_LENGTH_PRECISION)},-1`);
    ...
    private generateStorageArea(sa: StorageArea): void {
        const centroidX = sa.centroid ? formatNumber(sa.centroid.x, DEFAULT_COORD_PRECISION) : "";
        const centroidY = sa.centroid ? formatNumber(sa.centroid.y, DEFAULT_COORD_PRECISION) : "";
        this.addLine(`Storage Area=${formatNumber(sa.id, 0)}             ,${centroidX},${centroidY}`);
        if (sa.surfaceLine.length > 0) {
            ...
            sa.surfaceLine.forEach(coord => {
                this.addLine(`${formatNumber(coord.x, DEFAULT_COORD_PRECISION).padStart(17)} ${formatNumber(coord.y, DEFAULT_COORD_PRECISION).padStart(23)}`);
            });
        }
    ...
    private generateConnection(conn: Connection): void {
        this.addLine(`Connection=${formatNumber(conn.id,0)}             ,${conn.description || ""}`);
        if (conn.line.length > 0) {
            this.addLine(`Connection Line=${conn.line.length}`);
            this.addLines(formatCoordinatesToString(conn.line, 2, DEFAULT_COORD_PRECISION)); // Usually 2 pairs
        }
        ...
        if (conn.weirStationElevation.length > 0) {
            ...
            this.addLines(formatStaElevToString(conn.weirStationElevation, conn.weirStationElevation.length, DEFAULT_ELEV_PRECISION));
        }
    ...
    private generateGisInfo(): void {
        ...
        if (gi.rasterPath) this.addLine(`Geom Raster=${gi.rasterPath},True,image,, 0 `); // Assuming defaults for other parts
        this.addLine("Use User Specified Reach Order=0"); // Assuming
        if (this.geometry.reaches.length > 0) {
            const reachOrder = this.geometry.reaches.map(r => `${r.riverName.padEnd(16)},${r.reachName.padEnd(16)}`).join('');
        ...
        if (gi.xsCutLineLayer) this.addLine(`GIS Cross Section Layer=${gi.xsCutLineLayer}`);
        ...
        this.addLine("Write typescript browser code that will parse the provided file.  The file provided is HECRAS geometry file for a 2D model. The file will be uploaded by the user."); // This line from prompt, unusual for g-file
        this.addLine("");
        this.addLine("I want the code quality to be high, modular. Use specialized classes for each geometry type found.");
        this.addLine("");
