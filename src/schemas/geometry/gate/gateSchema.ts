...
import { parseMaybeFloat, parseMaybeInt, splitIntoTuples } from "../../../schema/parsingUtils"
import {
...
export const gateContextual = contextual(
  "gate",
  (lines, startIndex, _ctx) => {
    ...
    const trunnionHeight = parseMaybeFloat(parts[14])
    const submergedOrificeFlow = parseMaybeFloat(parts[15])
    const headReference = parseMaybeInt(parts[16]) // 0 = sill invert, 1 = center of opening
    const radialCoefficient = parseMaybeFloat(parts[17])
    const ratingCurveName = parseMaybeFloat(parts[18])
  ...
  (value, _ctx) => {
    ...
    const dataLine = [
      ...
      formatNullableNumber(value.trunnionHeight),
      formatNullableNumber(value.submergedOrificeFlow),
      ` ${value.headReference} `,
      formatNullableNumber(value.radialCoefficient),
