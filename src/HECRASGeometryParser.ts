...
import {
    parseCoordinates, parseStaElev, parseLineToNumbers,
    parseCommaSeparated, parseKeyValue, parseVolumeElevation
} from './utils';
import { HECRASGeometry, type ViewingRectangle, type GisInfo } from './models/geometry';
...
export class HecRasGeometryParser {
    ...
    private parseHeader(): void {
        ...
        while (line !== null) {
            if (line.startsWith('Geom Title=')) {
                this.geometry.title = parseKeyValue(line)?.value || null;
            } else if (line.startsWith('Program Version=')) {
                this.geometry.programVersion = parseKeyValue(line)?.value || null;
            } else if (line.startsWith('Viewing Rectangle=')) {
                const values = parseKeyValue(line)?.value.split(',').map(parseFloat);
                if (values && values.length === 4) {
                    this.geometry.viewingRectangle = {
                        minX: values[0], maxY: values[1], // Note HEC-RAS order
                        maxX: values[2], minY: values[3]
                    };
                }
    ...
    private parseReachData(reach: Reach): void {
        ...
        if (line?.startsWith('Reach XY=')) {
            ...
            while (pointsCollected < numPoints && this.currentIndex < this.lines.length) {
                const coordLine = this.lines[this.currentIndex];
                if (!coordLine || coordLine.trim() === "" || this.isNewSection(coordLine)) break;
    ...
    private isNewSection(line: string): boolean {
        const keywords = [
            'River Reach=', 'Type RM Length L Ch R =', 'Storage Area=', 'Connection=',
        ...
        while(line !== null && !this.isNewSection(line)) {
            if (line.startsWith('XS GIS Cut Line=')) {
                ...
                while(pointsCollected < numPoints && this.currentIndex < this.lines.length) {
                    ...
                    if (!staElevLine || this.isNewSection(staElevLine) || /^[A-Za-z#]/.test(staElevLine.trimStart())) break;
                ...
                this.advanceLine();
                for (let k = 0; k < numSegments && this.currentIndex < this.lines.length; ) {
                    const manningLine = this.lines[this.currentIndex];
                     if (!manningLine || this.isNewSection(manningLine) || /^[A-Za-z#]/.test(manningLine.trimStart())) break;
                    const values = parseLineToNumbers(manningLine);
                    for(let j=0; j < values.length; j+=3) { // 0 (dummy), station, nVal
                        if(j+2 < values.length) {
                             xs.manningSegments.push({ station: values[j+1], nValue: values[j+2], isDummy: values[j] === 0 });
                             k++;
                        }
                    }
                    this.advanceLine();
                ...
                const [left, right] = (parseKeyValue(line)?.value || "").split(',').map(parseFloat);
                xs.bankStations = { left: isNaN(left) ? null : left, right: isNaN(right) ? null : right };
                this.advanceLine();
            } else if (line.startsWith('Exp/Cntr=')) {
                const [exp, cntr] = (parseKeyValue(line)?.value || "").split(',').map(parseFloat);
                ...
                this.advanceLine(); 
                for(let k=0; k < numIneff && this.currentIndex < this.lines.length; k++) {
                    const ineffLine = this.lines[this.currentIndex];
                     if (!ineffLine || this.isNewSection(ineffLine) || /^[A-Za-z#]/.test(ineffLine.trimStart())) break;
                ...
                const isPermanent = (parseKeyValue(line)?.value || "F").toUpperCase() === 'T';
                // Apply to the last parsed ineffective flow area if any
                if (xs.ineffectiveFlowAreas.length > 0) {
                    xs.ineffectiveFlowAreas[xs.ineffectiveFlowAreas.length -1].isPermanent = isPermanent;
                }
                this.advanceLine();
            }
            else {
        ...
        while(line !== null && !this.isNewSection(line)) {
            if (line.startsWith('Lateral Weir WD=')) {
                ...
                while(pointsCollected < numPoints && this.currentIndex < this.lines.length) {
                    ...
                    if (!seLine || this.isNewSection(seLine) || /^[A-Za-z#]/.test(seLine.trimStart())) break;
            ...
            if (line.startsWith('Storage Area Surface Line=')) {
                ...
                while(pointsCollected < numPoints && this.currentIndex < this.lines.length) {
                    const surfLine = this.lines[this.currentIndex];
                     if (!surfLine || this.isNewSection(surfLine) || /^[A-Za-z#]/.test(surfLine.trimStart())) break;
                ...
                while(pointsCollected < numPoints && this.currentIndex < this.lines.length) {
                    ...
                    if (!volElevLine || this.isNewSection(volElevLine) || /^[A-Za-z#]/.test(volElevLine.trimStart())) break;
        ...
        while(line !== null && !this.isNewSection(line)) {
             if (line.startsWith('Connection Line=')) {
                ...
                while(pointsCollected < numPoints && this.currentIndex < this.lines.length) {
                    ...
                    if (!connLine || this.isNewSection(connLine) || /^[A-Za-z#]/.test(connLine.trimStart())) break;
    ...
    private parseGisInfo(): void {
        ...
        while(line !== null) {
            ...
            if (kv) {
                switch(kv.key) {
                    case 'Geom Raster': this.geometry.gisInfo.rasterPath = kv.value.split(',')[0]; break;
                    case 'GIS Units': this.geometry.gisInfo.units = kv.value; break;
                    case 'GIS DTM Type': this.geometry.gisInfo.dtmType = kv.value; break;
                    case 'GIS DTM': this.geometry.gisInfo.dtmPath = kv.value; break;
                    case 'GIS Stream Layer': this.geometry.gisInfo.streamLayer = kv.value; break;
                    case 'GIS Cross Section Layer': this.geometry.gisInfo.xsCutLineLayer = kv.value; break;
                    case 'GIS Map Projection': this.geometry.gisInfo.projection = kv.value; break;
                    case 'GIS Datum': this.geometry.gisInfo.datum = kv.value; break;
            ...
            if (line === null || line.trim() === "" || (!kv?.key.startsWith('GIS') && !kv?.key.startsWith('Geom Raster'))) {
                break;
            }
        }
    }
    ...
    public parse(fileContent: string): HECRASGeometry {
        ...
        while (this.currentIndex < this.lines.length) {
            ...
            if (line.startsWith('River Reach=')) {
                const [riverName, reachName] = (parseKeyValue(line)?.value || "").split(',').map(s => s.trim());
                ...
                if (type === 1 && currentReach) { // Cross Section
                    const xs = new CrossSection(rm);
                    if (parts.length > 2 && parts[2].trim() !== "") xs.lengthL = parseFloat(parts[2]);
                    if (parts.length > 3 && parts[3].trim() !== "") xs.lengthCh = parseFloat(parts[3]);
                    if (parts.length > 4 && parts[4].trim() !== "") xs.lengthR = parseFloat(parts[4]);
                    currentReach.crossSections.push(xs);
                    this.advanceLine();
                    this.parseCrossSectionData(xs);
                } else if (type === 6 && currentReach) { // Lateral Structure
            ...
            } else if (line.startsWith('Geom Raster=')) { // GIS info often at the end
                this.parseGisInfo();
            }
            else {
